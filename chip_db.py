	...
	with gopen(infile) as openfile:
		for line in openfile:
			...
			if not line[0] == '#':
				...
				for chr_idx, window_idx in zip(range(chr_start_idx, chr_end_idx+1), 
							range(window_start_idx, window_end_idx+1)):
...
def make_chr_breakpoints(size_file, resolution):
	...
	with open(size_file,'rU') as chromfile:
		for line in chromfile:
			...
			length = int(field[1])
			chr_sizes.append(length/resolution + int(length%resolution != 0) )
...
def read_wig(chip_file):
	...
	dfs = [g[1:].rename(
		columns={0:(g.iloc[0].values[0]).split()[1].split('=')[1]}).reset_index(drop=True) \
		for i, g in df.groupby(mask)]
	#
	# returns a dictionary mapping the chromosome name to the array in float32 and flattened
	return {dfs[idx].columns[0]: np.array(df).astype('float32').flatten() \
...
class chip_data:
	...
	def parse_tracks(self, tracks):
		...
		if 'all_hm' in tracks:
			for idx, avail_track in enumerate(self.avail_tracks):
				if ('Histone'+self.cell_type+'H').lower() in avail_track.lower():
					self.track_idxs.append(idx)
				if ('dnase'+self.cell_type).lower() in avail_track.lower():
					self.track_idxs.append(idx)
		if 'primary_hm' in tracks:
			for idx, avail_track in enumerate(self.avail_tracks):
				for mark in primary_marks:
					if ('Histone'+self.cell_type+mark).lower() in avail_track.lower():
						self.track_idxs.append(idx)
	...
	def get_data(self, chromosome, start_pos, end_pos, res=None, bin_method='mean', transform=None):
		...
		if not num_less == num_more:
			...
			#print self.chr_sizes, self.chr_starts
	...
	def add_track(self, track):
		...
		if '.wig' in track:
			# read_wig returns a dictionary of 
...
def main():
	...
	for track in args.data:
		track_name = track.split('/')[-1].split('.')[0]
		#print track, my_data.avail_tracks, track in my_data.avail_tracks
